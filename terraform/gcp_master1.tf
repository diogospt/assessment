resource "google_compute_instance" "master-1" {
  ...
  boot_disk {
    source      = google_compute_disk.master-1-boot-disk.id
    auto_delete = true
  }
  ...
  network_interface {
    ...
    network_ip = google_compute_address.address-master-1.address
...
resource "google_compute_address" "address-master-1" {
  name = "master-1"
...
resource "google_compute_disk" "master-1-boot-disk" {
  name  = "boot-disk-master-1"
