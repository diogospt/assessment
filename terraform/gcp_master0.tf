resource "google_compute_instance" "master-0" {
  ...
  boot_disk {
    source      = google_compute_disk.master-0-boot-disk.id
    auto_delete = true
  }
  ...
  network_interface {
    ...
    network_ip = google_compute_address.address-master-0.address
...
resource "google_compute_disk" "master-0-boot-disk" {
  name  = "boot-disk-master-0"
